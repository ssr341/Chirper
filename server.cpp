...
void serverAction(int connfd){
	...
	friendMutex.lock();
	postMutex.lock();
	ifstream userCheck("server1/users.txt");
	ifstream friendCheck("server1/friends.txt");
	ifstream postCheck("server1/posts.txt");

	// if any don't exist, get the replicate file
	if (!userCheck){
		repUserMutex.lock();
		userCheck.open("server2/users.txt");
		if (!userCheck)
			perror("User file missing!");
		ofstream temp("server1/users.txt");
		string line;
	...
	if (!friendCheck){
		repFriendMutex.lock();
		friendCheck.open("server2/friends.txt");
		if (!friendCheck)
			perror("Friend file missing!");
		ofstream temp("server1/friends.txt");
		string line;
	...
	if (!postCheck){
		repPostMutex.lock();
		postCheck.open("server2/posts.txt");
		if (!postCheck)
			perror("Post file missing!");
		ofstream temp("server1/posts.txt");
		string line;
	...
	if (login != NULL){
		...
		ifstream file("server1/users.txt");
		if (!file){
	...
	if (viewPosts != NULL){
		...
		ifstream fFile("server1/friends.txt");
		ifstream pFile("server1/posts.txt");
		if (!fFile && !pFile)
			perror("Connection failed, file does not exist");
		else{
	...
	if (personalPosts != NULL){
		...
		ifstream file("server1/posts.txt");
		if (!file)
			perror("Connection failed, file does not exist");
		else{
			char username[30] = " ";
	...
	if (postIt != NULL){
		...
		if (!pFile)
			...
			ofstream temp("server1/temp.txt");
			// get username
			char username[30] = " ";
			int count = 0;
			postIt = postIt + 7;
			...
			pFile.open("server2/posts.txt");
			temp.open("server2/temp.txt");
			while (getline(pFile, line)){
				char cline[1000];
				strcpy(cline, line.c_str());
				fprintf(stderr, "%s\n", cline);
	...
	if (reg != NULL){
		...
		// check to see if user file exists before parsing
		fstream file("server1/users.txt");
		if (!file)
			perror("Connection failed, file does not exist");
		else{
			char username[30] = " ";
	...
	if (friends != NULL){
		...
		ifstream file("server1/friends.txt");
		if (!file)
			perror("Connection failed, file does not exist");
		else{
			char username[30] = " ";
	...
	if (followReq != NULL){
		...
		ifstream fFile("server1/friends.txt");
		ifstream uFile("server1/users.txt");
		if (!fFile || !uFile)
			perror("Connection failed, file does not exist");
		else{
			// get username
